...
import com.musenkishi.wally.dataprovider.okhttp.OkHttpCustomUtil;
...
public class NetworkDataProvider {
  ...
  private Uri buildWallhavenUrl(int page, String path,
    ...
    builder.scheme("http")
        ...
        .appendQueryParameter(FilterBoardsKeys.PARAMETER_KEY,
            filterGroupsStructure.getBoardsFilter())
        .appendQueryParameter(FilterResolutionKeys.PARAMETER_KEY,
            filterGroupsStructure.getResolutionFilter().getValue())
        .appendQueryParameter(FilterPurityKeys.PARAMETER_KEY,
            filterGroupsStructure.getPurityFilter())
        .appendQueryParameter(FilterAspectRatioKeys.PARAMETER_KEY,
            filterGroupsStructure.getAspectRatioFilter().getValue())
  ...
  public String getDataSync(String url) {
    try {
      OkHttpClient client = OkHttpCustomUtil.getInstance();
  ...
  public void getData(String path, int index, FilterGroupsStructure filterGroupsStructure,
      final OnDataReceivedListener onDataReceivedListener) {
    Uri uri = buildWallhavenUrl(index, path, filterGroupsStructure);
    String url = uri.toString();
    ...
    try {
      ...
      if (onDataReceivedListener != null) {
        DataProviderError dataProviderError =
            new DataProviderError(DataProviderError.Type.NETWORK, 400, "MalformedURLException");
      ...
      if (onDataReceivedListener != null) {
        DataProviderError dataProviderError =
            new DataProviderError(DataProviderError.Type.NETWORK, 400, "IOException");
        onDataReceivedListener.onError(dataProviderError);
      }
      e.printStackTrace();
    }
  ...
  public void getData(String path, String query, String color, int index,
      FilterGroupsStructure filterGroupsStructure, OnDataReceivedListener onDataReceivedListener) {
    Uri uri = buildWallhavenUrl(index, path, filterGroupsStructure);
    if (color != null) {
