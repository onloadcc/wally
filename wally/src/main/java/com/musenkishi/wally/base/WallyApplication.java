...
public class WallyApplication extends Application {
  ...
  private void checkVersionInstalled(int currentVersion) {
    ...
    int latestVersion = getDataProviderInstance().getSharedPreferencesDataProviderInstance()
    ...
    if (latestVersion < currentVersion) {
      ...
      if (latestVersion == defaultVersion) {
        getDataProviderInstance().getSharedPreferencesDataProviderInstance()
            .getPrefs()
            .edit()
            .clear()
            .apply();
  ...
  private void startCrashLoggingIfUserAccepted() {
    if (getDataProviderInstance().getSharedPreferencesDataProviderInstance().getAppStartCount()
        >= FAMILIAR_USER_COUNT) {
      switch (getDataProviderInstance().getSharedPreferencesDataProviderInstance()
  ...
  public static DataProvider getDataProviderInstance() {
    if (dataProvider == null) {
      dataProvider = new DataProvider(getContext(), new ExceptionReporter.OnReportListener() {
        @Override public void report(Class fromClass, String reason, String exceptionMessage) {
          if (getDataProviderInstance().getSharedPreferencesDataProviderInstance()
              .hasUserApprovedCrashLogging()
  ...
  public static FilterGroupsStructure getFilterSettings() {
    ...
    filterGroupsStructure.setTimespanFilter(
        dataProvider.getTimespan(FilterTimeSpanKeys.PARAMETER_KEY));
    ...
    filterGroupsStructure.setAspectRatioFilter(
        dataProvider.getAspectRatio(FilterAspectRatioKeys.PARAMETER_KEY));
    filterGroupsStructure.setResOptFilter(
        dataProvider.getResolutionOption(FilterResOptKeys.PARAMETER_KEY));
    filterGroupsStructure.setResolutionFilter(
        dataProvider.getResolution(FilterResolutionKeys.PARAMETER_KEY));
  ...
  public static HashMap<String, Object> readMessages(String fragmentTag) {
    ...
    if (fragmentTag != null) {
      if (fragmentTag.equals(SearchFragment.TAG) && !getSearchFragmentMessages().isEmpty()) {
